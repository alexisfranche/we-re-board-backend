...
def get_active_events():
    try:
        active_events = Event.query.filter(and_(Event.status == EventStatus.Active.value, dt.strptime(Event.datetime.isoformat(), '%Y-%m-%d %H:%M:%S')>dt.now())).all()
    except:
...
def get_events_by_category(game):
    ...
    try:
        category_events =  all_events = Event.query.filter(and_(Event.game == game, Event.status == EventStatus.Active.value, dt.strptime(Event.datetime.isoformat(), '%Y-%m-%d %H:%M:%S')>dt.now())).all()
    except:
